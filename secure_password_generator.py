...
default_other_special_chars = "`&*=|;\"<,>§£øéèàçùαβγδεζηΘθλμνξΠπρΣτυΦφΨψΩω"
...
simple_config = {
    ...
    "other_special_chars": default_other_special_chars,
    "default": "yyynn",
    "range_min": 8,
    "range_max": 15
}
...
def change_config():
    ...
    global default_other_special_chars
    ...
    print("6. Edit other special characters")
...
1. Simple config : passwords will not contain special characters. Entering r when size is asked will generate a password of a lenght between 8 and 15. THIS CONFIGURATION IS NOT RECOMMENDED.
2. Usual config : passwords will contain special characters. Entering r when size is asked will generate a password of a lenght between 12 and 30. This config is the default one.
3. Strong config : passwords will contain special characters. Entering r when size is asked will generate a password of a lenght between 30 and 50. When long passwords are accepted, this configuration is recommended.
4. Very strong config : passwords will contain special characters and other special characters (e.g. greek letters). Entering r when size is asked will generate a password of a lenght between 50 and 100. Please note that many systems will not accept the other special characters and may not accept such long passwords.
        """)
...
To reset the set to its default value : *reset
        ...
        while 1:
            print("The current set of special characters is :   " + config["other_special_chars"])
            data = input("Edit with : ")

            if data.find("*exit") > -1:
                break
            elif data.find("*reset") > -1:
                config["other_special_chars"] = default_other_special_chars
            else:
                edited_data = ""
                for char in data:
                ...
                config["other_special_chars"] = edit_chain(config["other_special_chars"], edited_data)
    elif choice == 7:
        choices = ["uppercases", "lowercases", "digits", "special characters", "other sepcial characters"]
        new_settings = ""
...
def generate_passwords():
    ...
    choice = input(
        "Should I use the default settings ? Current {} (in order : use uppercases, lowercases, digits, special character, other special characters) : (y/n) ".format(
            config["default"]))
    settings = ""
    if choice == 'y':
        settings = config["default"]
    else:
        choices = ["uppercases", "lowercases", "digits", "special characters", "other special characters"]
    ...
    if settings[4] == 'y':
        chars += config["other_special_chars"]
...
You can find here the number of possible passwords assuming your actual settings.
    ...
    choice = input(
        "Should I use the default settings ? Current : {} (in order : use uppercases, lowercases, digits, special characters, other special characters) : (y/n) ".format(
            config["default"]))
    settings = ""
    if choice == 'y':
        settings = config["default"]
    ...
    if settings[4] == 'y':
        chars_len += len(config["other_special_chars"])
    ...
    number_of_passwords = chars_len ** size
